...
test_that("apply.WINGS works for provided example and returns the expected structure", {
  ...
  expect_type(result$r_plus_c, "double")
  expect_type(result$r_minus_c, "double")
  expect_type(result$weights, "double")
  #'
  expect_length(result$r_plus_c, nrow(mat))
  expect_length(result$r_minus_c, nrow(mat))
  expect_length(result$weights, nrow(mat))
})
